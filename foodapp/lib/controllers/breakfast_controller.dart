...
import 'package:foodapp/data/repository/breakfast_repo.dart';
...
class BreakfastController extends GetxController{
  final BreakfastRepo breakfastRepo;
  BreakfastController({required this.breakfastRepo});
  List<dynamic> _breakfastList=[];
  List<dynamic> get breakfastList => _breakfastList;
  List dummyList=[];
  ...
  Future<void> getBreakfastList() async {
    Response response = await breakfastRepo.getBreakfastList();
    if(response.statusCode==200){
      // print("Got products");
      _breakfastList=[];
      _breakfastList.addAll(Product.fromJson(response.body).products);
